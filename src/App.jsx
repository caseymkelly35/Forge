...
function HomeScreen({ user, history, templates, squadMembers, activeProgramRow, allPrograms, onStartBuild, onLoadTemplate, onStartFreestyle, onOpenHistory, onOpenSocial, onOpenPrograms, onStartTodayWorkout }) {
  ...
  const activeProgram = activeProgramRow ? (allPrograms || PROGRAMS).find((p) => p.id === activeProgramRow.program_id) : null;
  const todayInfo = activeProgramRow ? getTodayProgramDay(activeProgramRow, allPrograms) : null;
...
function getTodayProgramDay(activeProgramRow, allPrograms) {
  if (!activeProgramRow) return null;
  const program = (allPrograms || PROGRAMS).find((p) => p.id === activeProgramRow.program_id);
...
function dbCustomProgramToLocal(row) {
  return {
    id: row.id,
    name: row.name,
    goal: "Custom",
    color: "#8B5CF6",
    isCustom: true,
    basedOnPreset: row.based_on_preset,
    durationWeeks: row.duration_weeks,
    description: `Custom program${row.based_on_preset ? " · started from a preset" : ""}`,
    progressionNote: "This is your own program — adjust it anytime as your training evolves.",
    days: row.days || [],
  };
}

async function fetchCustomPrograms(token) {
  const rows = await supabaseRest("custom_programs?select=*&order=created_at.desc", { token });
  return (rows || []).map(dbCustomProgramToLocal);
}

async function createCustomProgram(token, userId, program) {
  const [row] = await supabaseRest("custom_programs", {
    method: "POST",
    token,
    body: {
      owner_id: userId,
      name: program.name,
      based_on_preset: program.basedOnPreset || null,
      duration_weeks: program.durationWeeks,
      days: program.days,
    },
  });
  return dbCustomProgramToLocal(row);
}

async function updateCustomProgram(token, programId, patch) {
  const body = {};
  if ("name" in patch) body.name = patch.name;
  if ("durationWeeks" in patch) body.duration_weeks = patch.durationWeeks;
  if ("days" in patch) body.days = patch.days;
  const [row] = await supabaseRest(`custom_programs?id=eq.${programId}`, { method: "PATCH", token, body });
  return dbCustomProgramToLocal(row);
}

async function deleteCustomProgram(token, programId) {
  await supabaseRest(`custom_programs?id=eq.${programId}`, { method: "DELETE", token });
}
...
function SocialScreen({ user, history, onBack, onSignOut, onStartSquad, onChangeAvatar }) {
  ...
  const inviteGroupToLift = (group) => {
    setInvitedToast(`${group.name} (${group.memberIds.length})`);
    setTimeout(() => setInvitedToast(null), 1800);
  };
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, paddingBottom: 40 }}>
      ...
      <div style={{ padding: "18px 20px 0" }}>
        ...
        <div style={{ marginTop: 26 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
            ...
            <button
              onClick={() => setShowGroupEditor({ name: "", memberIds: [] })}
              className="fg-mono"
              style={{ background: "none", border: `1px solid ${C.line}`, borderRadius: 8, padding: "5px 10px", color: C.textLo, fontSize: 11, display: "flex", alignItems: "center", gap: 4 }}
            >
          ...
          {groups.length === 0 ? (
            <div className="fg-mono" style={{ color: C.textLo, fontSize: 12, lineHeight: 1.5 }}>
              Optional — group friends who train together (like "Football Team" or just the two of you) for quick access. Not required.
            </div>
          ) : (
            <div style={{ display: "flex", gap: 8, overflowX: "auto", paddingBottom: 4 }}>
              {groups.map((g) => (
                <div
                  ...
                  className="fg-tap"
                  style={{ minWidth: 140, background: C.bgCard, border: `1px solid ${C.line}`, borderRadius: 12, padding: 12, cursor: "pointer", flexShrink: 0 }}
                >
                  <div className="fg-display" style={{ color: C.textHi, fontSize: 15, fontWeight: 600 }}>{g.name}</div>
                  <div className="fg-mono" style={{ color: C.textLo, fontSize: 11, marginTop: 2 }}>
      ...
      {viewingGroupId && (() => {
        const g = groups.find((x) => x.id === viewingGroupId);
        if (!g) return null;
        const members = friends.filter((f) => g.memberIds.includes(f.id));
        return (
          <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", zIndex: 65, display: "flex", alignItems: "flex-end" }} onClick={() => setViewingGroupId(null)}>
            <div onClick={(e) => e.stopPropagation()} className="fg-sheet-in" style={{ width: "100%", background: C.bgRaised, borderTop: `1px solid ${C.line}`, borderRadius: "20px 20px 0 0", padding: 22, maxHeight: "75vh", overflowY: "auto" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                <div className="fg-display" style={{ color: C.textHi, fontSize: 20, fontWeight: 700 }}>{g.name}</div>
                <button onClick={() => setViewingGroupId(null)} style={{ background: "none", border: "none" }}>
                  <X size={20} color={C.textLo} />
                </button>
              </div>

              {members.length === 0 ? (
                <div className="fg-mono" style={{ color: C.textLo, fontSize: 13, textAlign: "center", padding: 20, marginBottom: 16 }}>
                  No members in this group yet.
                </div>
              ) : (
                <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 20 }}>
                  {members.map((f) => (
                    <div key={f.id} style={{ display: "flex", alignItems: "center", gap: 10, background: C.bgCard, border: `1px solid ${C.line}`, borderRadius: 10, padding: "10px 12px" }}>
                      <Avatar name={f.name} size={32} />
                      <div className="fg-display" style={{ flex: 1, color: C.textHi, fontSize: 14, fontWeight: 600 }}>{f.name}</div>
                      <div style={{ width: 8, height: 8, borderRadius: "50%", background: f.online ? "#22C55E" : C.textLo }} />
                    </div>
                  ))}
                </div>
              )}
              ...
              <button
                onClick={() => inviteGroupToLift(g)}
                className="fg-display"
                style={{ width: "100%", background: C.blue, border: "none", borderRadius: 12, padding: "14px", color: "white", fontWeight: 700, fontSize: 15, marginBottom: 10 }}
              >
                Invite Group to Lift
              </button>
              <div style={{ display: "flex", gap: 10 }}>
                <button
                  onClick={() => { setShowGroupEditor(g); setViewingGroupId(null); }}
                  className="fg-display"
                  style={{ flex: 1, background: "transparent", border: `1px solid ${C.line}`, borderRadius: 10, padding: "12px", color: C.textHi, fontWeight: 600, fontSize: 13 }}
                >
                  Edit
                </button>
                <button
                  onClick={() => { setGroups((list) => list.filter((x) => x.id !== g.id)); setViewingGroupId(null); }}
                  className="fg-display"
                  style={{ flex: 1, background: "transparent", border: `1px solid #EF4444`, borderRadius: 10, padding: "12px", color: "#EF4444", fontWeight: 600, fontSize: 13 }}
                >
                  Delete
                </button>
              </div>
      ...
      {showGroupEditor && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", zIndex: 66, display: "flex", alignItems: "flex-end" }} onClick={() => setShowGroupEditor(null)}>
          <div onClick={(e) => e.stopPropagation()} className="fg-sheet-in" style={{ width: "100%", background: C.bgRaised, borderTop: `1px solid ${C.line}`, borderRadius: "20px 20px 0 0", padding: 22, maxHeight: "80vh", overflowY: "auto" }}>
            <div className="fg-display" style={{ color: C.textHi, fontSize: 20, fontWeight: 700, marginBottom: 16 }}>
              {showGroupEditor.id ? "Edit Group" : "New Group"}
            </div>
            <input
              value={showGroupEditor.name}
              onChange={(e) => setShowGroupEditor((g) => ({ ...g, name: e.target.value }))}
              placeholder="Group name (e.g. Football Team)"
              className="fg-mono"
              style={{ width: "100%", background: C.bgCard, border: `1px solid ${C.line}`, borderRadius: 10, padding: "13px 14px", color: C.textHi, fontSize: 15, marginBottom: 18 }}
              autoFocus
            />
            <div className="fg-mono" style={{ color: C.textLo, fontSize: 11, letterSpacing: "0.08em", marginBottom: 10 }}>SELECT MEMBERS</div>
            {friends.length === 0 ? (
              <div className="fg-mono" style={{ color: C.textLo, fontSize: 13, textAlign: "center", padding: 20, marginBottom: 16 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 20 }}>
                {friends.map((f) => {
                  const selected = showGroupEditor.memberIds.includes(f.id);
                  return (
                    <div
                      key={f.id}
                      onClick={() => setShowGroupEditor((g) => ({ ...g, memberIds: selected ? g.memberIds.filter((id) => id !== f.id) : [...g.memberIds, f.id] }))}
                      className="fg-tap"
            ...
            <button
              onClick={() => {
                const name = showGroupEditor.name.trim() || "Untitled Group";
                if (showGroupEditor.id) {
                  setGroups((list) => list.map((g) => (g.id === showGroupEditor.id ? { ...g, name, memberIds: showGroupEditor.memberIds } : g)));
                } else {
                  setGroups((list) => [...list, { id: `grp_${Date.now()}`, name, memberIds: showGroupEditor.memberIds }]);
                }
                setShowGroupEditor(null);
              }}
              className="fg-display"
              style={{ width: "100%", background: C.blue, border: "none", borderRadius: 12, padding: "15px", color: "white", fontWeight: 700, fontSize: 16 }}
            >
              Save Group
            </button>
          </div>
...
function ProgramEditorScreen({ program, templates, onCancel, onSave }) {
  const [name, setName] = useState(program.name || "My Program");
  const [durationWeeks, setDurationWeeks] = useState(program.durationWeeks || 4);
  const [days, setDays] = useState(() => {
    const existing = program.days || [];
    return Array.from({ length: 7 }, (_, day) => existing.find((d) => d.day === day) || { day, type: "rest" });
  });
  const [pickingDay, setPickingDay] = useState(null);

  const toggleDayType = (day) => {
    setDays((list) => list.map((d) => (d.day === day ? (d.type === "rest" ? { day, type: "workout" } : { day, type: "rest" }) : d)));
  };

  const assignTemplate = (day, template) => {
    setDays((list) =>
      list.map((d) =>
        d.day === day
          ? { day, type: "workout", label: template.name, config: template.config, buildItems: template.buildItems, burnoutItems: template.burnoutItems || [] }
          : d
      )
    );
    setPickingDay(null);
  ...
  const handleSave = () => {
    onSave({
      ...
      basedOnPreset: program.basedOnPreset || null,
      durationWeeks: Number(durationWeeks) || 4,
      // a "Workout" day with no template ever assigned is functionally
      // identical to a rest day — save it as one instead of an empty session
      days: days.map((d) => (d.type === "workout" && !d.buildItems ? { day: d.day, type: "rest" } : d)),
    });
  };
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, paddingBottom: 110 }}>
      ...
      <div style={{ padding: "22px 20px 6px", display: "flex", alignItems: "center", gap: 12 }}>
        ...
        <h1 className="fg-display" style={{ color: C.textHi, fontSize: 22, fontWeight: 700, margin: 0 }}>
          {program.id ? "Edit Program" : "New Program"}
        </h1>
      </div>

      <div style={{ padding: "14px 20px 0" }}>
        <div className="fg-mono" style={{ color: C.textLo, fontSize: 11, letterSpacing: "0.08em", marginBottom: 8 }}>PROGRAM NAME</div>
        <input
          value={name}
          onChange={(e) => setName(e.target.value)}
          className="fg-mono"
        ...
        <div className="fg-mono" style={{ color: C.textLo, fontSize: 11, letterSpacing: "0.08em", marginBottom: 8 }}>DURATION (WEEKS)</div>
        <div style={{ marginBottom: 22 }}>
          <DialInput value={Number(durationWeeks)} onChange={setDurationWeeks} min={1} max={16} step={1} unit=" wks" />
        </div>
            ...
            return (
              <div key={dayNum} style={{ background: C.bgCard, border: `1px solid ${isRest ? C.line : "#8B5CF666"}`, borderRadius: 12, padding: "12px 14px" }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: isRest ? 0 : 10 }}>
                  <span className="fg-display" style={{ color: C.textHi, fontSize: 15, fontWeight: 600 }}>{DAY_NAMES[dayNum]}</span>
                  <button
                    onClick={() => toggleDayType(dayNum)}
                    className="fg-mono"
                    style={{ background: "none", border: `1px solid ${C.line}`, borderRadius: 8, padding: "5px 10px", color: isRest ? C.textLo : "#C4B5FD", fontSize: 11 }}
                  >
                    {isRest ? "Rest" : "Workout"}
                  </button>
                </div>
                {!isRest && (
                  <div
                    onClick={() => setPickingDay(dayNum)}
                    className="fg-tap"
                    style={{ cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "space-between", background: C.bg, border: `1px dashed ${C.line}`, borderRadius: 8, padding: "10px 12px" }}
                  >
                    <span className="fg-mono" style={{ color: d.label ? C.textHi : C.textLo, fontSize: 13 }}>{d.label || "Choose a template..."}</span>
      ...
      <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, padding: 16, background: `linear-gradient(0deg, ${C.bg} 60%, transparent)` }}>
        <button onClick={handleSave} className="fg-display" style={{ width: "100%", background: "#8B5CF6", border: "none", borderRadius: 14, padding: "16px", color: "white", fontWeight: 700, fontSize: 17 }}>
          Save Program
        </button>
      </div>

      {pickingDay !== null && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", zIndex: 70, display: "flex", alignItems: "flex-end" }} onClick={() => setPickingDay(null)}>
          <div onClick={(e) => e.stopPropagation()} className="fg-sheet-in" style={{ width: "100%", maxHeight: "70vh", overflowY: "auto", background: C.bgRaised, borderTop: `1px solid ${C.line}`, borderRadius: "20px 20px 0 0", padding: 20 }}>
            <div className="fg-display" style={{ color: C.textHi, fontSize: 18, fontWeight: 700, marginBottom: 14 }}>
              Choose a template for {DAY_NAMES[pickingDay]}
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {(templates || []).map((t) => (
                <div
                  key={t.id}
                  onClick={() => assignTemplate(pickingDay, t)}
                  className="fg-tap"
...
function ProgramsScreen({ activeProgramRow, allPrograms, customPrograms, templates, onBack, onStartProgram, onCancelProgram, onStartTodayWorkout, onSaveCustomProgram, onDeleteCustomProgram }) {
  const [viewing, setViewing] = useState(null); // a program being previewed
  const [editingProgram, setEditingProgram] = useState(null); // a program being edited/created
  const [confirmingCancel, setConfirmingCancel] = useState(false);
  const [confirmingDeleteId, setConfirmingDeleteId] = useState(null);

  const activeProgram = activeProgramRow ? allPrograms.find((p) => p.id === activeProgramRow.program_id) : null;
  const todayInfo = activeProgramRow ? getTodayProgramDay(activeProgramRow, allPrograms) : null;
    ...
    return (
      <ProgramEditorScreen
        ...
        onSave={async (program) => {
          const saved = await onSaveCustomProgram(program);
      ...
      <div style={{ minHeight: "100vh", background: C.bg, paddingBottom: 30 }}>
        ...
        <div style={{ padding: "12px 20px 0" }}>
          ...
          <button
            ...
            className="fg-display"
            style={{ width: "100%", background: p.color, border: "none", borderRadius: 14, padding: "16px", color: "white", fontWeight: 700, fontSize: 16, marginBottom: 10 }}
          >
            Start This Program
          </button>
          ...
          {p.isCustom ? (
            <div style={{ display: "flex", gap: 10 }}>
              <button
                ...
                className="fg-display"
                style={{ flex: 1, background: "transparent", border: `1px solid ${C.line}`, borderRadius: 12, padding: "13px", color: C.textHi, fontWeight: 600, fontSize: 14 }}
              >
                Edit
              </button>
              {confirmingDeleteId === p.id ? (
                <button
                  onClick={() => { onDeleteCustomProgram(p.id); setConfirmingDeleteId(null); setViewing(null); }}
                  className="fg-display"
                  style={{ flex: 1, background: "#EF4444", border: "none", borderRadius: 12, padding: "13px", color: "white", fontWeight: 700, fontSize: 14 }}
                >
                  Confirm Delete
                </button>
              ) : (
                <button
                  ...
                  className="fg-display"
                  style={{ flex: 1, background: "transparent", border: `1px solid #EF4444`, borderRadius: 12, padding: "13px", color: "#EF4444", fontWeight: 600, fontSize: 14 }}
                >
                  Delete
                </button>
              )}
            ...
            <button
              onClick={() => setEditingProgram({ name: `${p.name} (Custom)`, basedOnPreset: p.id, durationWeeks: p.durationWeeks, days: p.days })}
              className="fg-display"
              style={{ width: "100%", background: "transparent", border: `1px solid #8B5CF6`, borderRadius: 12, padding: "13px", color: "#C4B5FD", fontWeight: 600, fontSize: 14, display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}
            >
              <LayoutList size={15} /> Customize a Copy
            </button>
          )}
        </div>
      </div>
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, paddingBottom: 30 }}>
      ...
      <div style={{ padding: "10px 20px 0" }}>
        ...
        <button
          onClick={() => setEditingProgram({ name: "My Program", durationWeeks: 4, days: [] })}
          className="fg-display"
          style={{
            width: "100%", background: "transparent", border: `1px dashed #8B5CF6`, borderRadius: 14,
            padding: "14px", color: "#C4B5FD", fontWeight: 600, fontSize: 15, marginBottom: 22,
            display: "flex", alignItems: "center", justifyContent: "center", gap: 8,
          }}
        >
        ...
        {customPrograms.length > 0 && (
          <>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 22 }}>
              {customPrograms.filter((p) => p.id !== activeProgram?.id).map((p) => (
                <div
                  key={p.id}
                  onClick={() => setViewing(p)}
                  className="fg-tap"
                  style={{ background: C.bgCard, border: `1px solid #8B5CF644`, borderRadius: 14, padding: 16, cursor: "pointer" }}
                >
                  ...
                  <div style={{ display: "flex", gap: 8 }}>
                    <Chip label="Custom" active color="#8B5CF6" />
                    <Chip label={`${p.durationWeeks} weeks`} active color={C.textLo} />
                  </div>
                </div>
              ))}
...
export default function App() {
  ...
  const handleAuthed = async (newSession) => {
    ...
    try {
      const [cloudHistory, cloudTemplates, profileExtras, program, customProgs] = await Promise.all([
        ...
        fetchCustomPrograms(newSession.access_token),
      ]);
      setHistory(cloudHistory);
      setTemplates([...PRESET_TEMPLATES, ...cloudTemplates]);
      setNeedsOnboarding(!profileExtras.has_onboarded);
  ...
  const saveCustomProgram = async (program) => {
    try {
      const token = await getValidToken(authSession, setAuthSession);
      if (!token) throw new Error("Not signed in");
      if (program.id && customPrograms.some((p) => p.id === program.id)) {
        const updated = await updateCustomProgram(token, program.id, program);
        setCustomPrograms((list) => list.map((p) => (p.id === program.id ? updated : p)));
        return updated;
      } else {
        const created = await createCustomProgram(token, authSession.user.id, program);
        setCustomPrograms((list) => [created, ...list]);
        return created;
      }
    } catch (e) {
      setCloudError("Couldn't save your program — check your connection and try again.");
      return null;
    }
  };

  const removeCustomProgram = async (programId) => {
    setCustomPrograms((list) => list.filter((p) => p.id !== programId)); // optimistic
    try {
      const token = await getValidToken(authSession, setAuthSession);
      if (!token) throw new Error("Not signed in");
      await deleteCustomProgram(token, programId);
    } catch (e) {
      setCloudError("Deleted locally, but couldn't sync to the server.");
    }
  };
  ...
  if (view === "squad") {
    ...
    screen = (
      <ProgramsScreen
        ...
        onSaveCustomProgram={saveCustomProgram}
        onDeleteCustomProgram={removeCustomProgram}
      />
    );
  } else if (view === "social") {
    screen = (
